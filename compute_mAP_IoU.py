...
#    gtfolder ='../mAP-IoU_testdata/ground-truth'
    ...
    predictfolder = os.path.abspath(os.path.join(root_path, 'output/dense/'))
    gtfolder = os.path.abspath(os.path.join(root_path, 'data/VOCDevkit2007/VOC2007/Annotations_txt/'))
    resultsfolder = os.path.abspath(os.path.join(root_path, 'output/results/'))
...
ground_truth_files_list = os.listdir(gtfolder)
...
#    lines_list = file_lines_to_list(txt_file)
    lines_list = file_lines_to_list(os.path.join(gtfolder, txt_file))
...
predicted_files_list = os.listdir(predictfolder)
#predicted_files_list = os.path.join(predicted_files_list , file_id + ".txt")
...
#        lines = file_lines_to_list(txt_file)
        lines = file_lines_to_list(os.path.join(predictfolder, txt_file))
        if lines==[]:
            continue
        else:
            for line in lines:
...
for txt_file in predicted_files_list:
    # get lines to list
    lines_list = file_lines_to_list(os.path.join(predictfolder, txt_file))
    for line in lines_list:
...
my_dictionary = {
    "EADChallenge2019":{
            "mAP":{"value": (mAP*100)},
            "IoU":{"value": (mIoU*100)},
            "score":{"value": (0.6 * mAP * 100 + 0.4 * mIoU * 100)},
            "mAP_specularity": {"value": (ap_dictionary['specularity'] * 100)},
            "mAP_contrast": {"value": (ap_dictionary['contrast'] * 100)},
            "mAP_saturation": {"value": (ap_dictionary['saturation'] * 100)},
            "mAP_blur":{"value": (ap_dictionary['blur'] * 100)},
            "mAP_instrument":{"value": (ap_dictionary['instrument'] * 100)},
            "mAP_bubbles":{"value": (ap_dictionary['bubbles'] * 100)},
            "mAP_artifact":{"value": (ap_dictionary['artifact'] * 100)},
            "iou_specularity": {"value": (iou_dictionary['specularity'] * 100)},
            "iou_contrast": {"value": (iou_dictionary['contrast'] * 100)},
            "iou_saturation": {"value": (iou_dictionary['saturation'] * 100)},
            "iou_blur":{"value": (iou_dictionary['blur'] * 100)},
            "iou_instrument":{"value": (iou_dictionary['instrument'] * 100)},
            "iou_bubbles":{"value": (iou_dictionary['bubbles'] * 100)},
            "iou_artifact":{"value": (iou_dictionary['artifact'] * 100)},
            "IoU_mAP_ratio":{"value": (ratioPass)}
        }
}
