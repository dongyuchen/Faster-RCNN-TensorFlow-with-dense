...
class vgg16(Network):
    ...
    def build_network(self, sess, is_training=True):
        with tf.variable_scope('vgg_16', 'vgg_16'):
            ...
            if cfg.FLAGS.initializer == "truncated":
                initializer = tf.truncated_normal_initializer(mean=0.0, stddev=0.01)#生成截断正态分布的随机数，它有四个参数（mean=0.0, stddev=1.0, seed=None, dtype=dtypes.float32)，分别用于指定均值、标准差、随机数种子和随机数的数据类型
    ...
    def build_head(self, is_training):
        ...
        return net
    def built_grad_cam(self, net, is_training, initializer):
        
        pool5 = slim.max_pool2d(net, [2, 2], padding='SAME', scope='pool5')
    ...
    def build_rpn(self, net, is_training, initializer):
        ...
        # Build anchor component  生成九个框的函數
        ...
        # 二分类和回归是并行的，于是用同样1*1得卷积去操作原来的feature map
        # 生成长度为4*k，即_num_anchors * 4得长度
    ...
    def build_predictions(self, net, rois, is_training, initializer, initializer_bbox):
        ...
        # Crop image ROIs 构建固定大小的rois窗口
        pool5 = self._crop_pool_layer(net, rois, "pool5")
        pool5_flat = slim.flatten(pool5, scope='flatten')

        # Fully connected layers 建立全连接层
        fc6 = slim.fully_connected(pool5_flat, 4096, scope='fc6')
        if is_training:
            fc6 = slim.dropout(fc6, keep_prob=0.5, is_training=True, scope='dropout6')
        ...
        # Scores and predictions 通过fc7进行box框的分类
